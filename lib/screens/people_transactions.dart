...
class _PeopleTScreenState extends State<PeopleTScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    List<Transaction> tList = user.transactions
        .where((element) => element.toOrFromId == widget.id)
        .toList();
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        centerTitle: true,
        bottom: (tCredit == 0 && tDebit == 0 && leftInterest == 0)
            ? null
            : AppBar(
                elevation: 0,
                backgroundColor: Colors.blueGrey[800],
      ...
      body: (tCredit == 0 && tDebit == 0 && leftInterest == 0)
          ? Center(
              child: Text(
                'No Transactions Yet!!\nTry Add Some.',
                style: TextStyle(
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () {
          showModalBottomSheet(
            ...
            isDismissible: true,
            builder: (context) => AddTrx(
              user,
              selectedP: widget.index,
            ),
          );
        },
