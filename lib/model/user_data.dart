...
class UserData with ChangeNotifier {
  ...
  List<People> people = [];
  List<String> methods = ['Cash', 'Credit Card', 'Debit Card', 'UPI', 'Check'];
  ...
  _fetchPrefs() async {
    ...
    if (result[0]) {
      ...
      notifyListeners();
      print('user data found in shared preferences');
      if (result[5].isAfter(DateTime.now())) {
        _isLogedIn = true;
        notifyListeners();
        print('And Expiry data is not crossed yet.');
        bool res = await _fetchData(result[5], result[4]);
        if (res) {
          print('successfully fetched the transaction data');
        } else {
          print('error in fetching transaction data');
        }
      } else {
        print('Expiry data is crossed and trying to login again.');
  ...
  _fromJson() async {
    ...
    Map<String, dynamic> data =
        json.decode(prefs.getString('userData') ?? '{"error": "No Data"}');
    // print(data);
  ...
  _fetchData(_expireDate, _id) async {
    try {
      ...
      // print(fUserData);
      _saveData(fUserData['name'] ?? 'NULL', fUserData['email'] ?? 'NULL',
          fUserData['password'] ?? 'NULL', _id, _expireDate);
      ...
      fPeople.forEach((key, value) {
        this
            .people
            .add(People(key, value['name'], value['address'], value['number']));
      });
      isFechingData = false;
      notifyListeners();
      return true;
  ...
  addPeople(String nameA, String addressA, String numberA) async {
    ...
    people
        .add(People(json.decode(data.body)['name'], nameA, addressA, numberA));
    notifyListeners();
  }
  ...
  deletePeople(String _id) async {
    ...
    this
        .transactions
        .where((element) => element.toOrFromId == _id)
